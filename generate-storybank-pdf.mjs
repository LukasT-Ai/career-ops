...
const md = await readFile(resolve(__dirname, 'interview-prep/story-bank-paulina.md'), 'utf8');
...
const htmlPath = resolve(outputDir, 'story-bank.html');
...
const pdfPath = resolve(outputDir, 'Interview-Story-Bank-Paulina-Kaiser.pdf');
...
if (process.argv.includes('--send')) {
  ...
  const lebenslaufPath = resolve(outputDir, 'Lebenslauf-Dr-Paulina-Kaiser.pdf');

  const attachments = [
    {
      filename: 'Interview-Story-Bank-Paulina-Kaiser.pdf',
      path: pdfPath,
  ...
  console.log('Email sent to paulinakaiser@gmail.com with PDF attachments');
  transport.close();
} else {
